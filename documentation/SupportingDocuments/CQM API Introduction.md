...
| QoS session | A time-bounded instantiation of a QoS profile for one or more application flows associated with a device. | It explains temporary QoS treatment, either requested on demand (using the `quality-on-demand` API) or in-future (using one of the reservation APIs). |
...
**Note on terminology:** This document uses "RESERVED" as the unified term for committed-but-not-activated states. However, the underlying CAMARA API specifications use different models: `qos-booking` uses SCHEDULED as its booking status, `qos-booking-and-assignment` uses BOOKING_ACCEPTED (in statusInfo) for the equivalent state, and `dedicated-network` uses RESERVED. Cross-API harmonization of these state models belongs in the API specifications.
