...
**Note on terminology:** This document uses "RESERVED" as the unified term for the state where the CSP has committed resources but the service is not yet activated. The underlying CAMARA API specifications currently use different names for this equivalent state: `dedicated-network` (DedicatedNetworks) uses RESERVED, while `qos-booking` and `qos-booking-and-assignment` (QoSBooking) use SCHEDULED. This terminology will be harmonized in a future CAMARA meta-release.
