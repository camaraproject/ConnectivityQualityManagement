...
### The APIs within the Connectivity Quality Management subproject
...
**Note:** `dedicated-network-areas` will move to the separate [NetworkServiceAreas](https://github.com/camaraproject/NetworkServiceAreas) repository in a future release. API status definitions follow the [CAMARA API Overview](https://camaraproject.org/api-overview/).

A detailed description and grouping of the CQM APIs is provided in [Section 5. CQM API Introduction and Grouping](#5-cqm-api-introduction-and-grouping).
...
| QoS session | A time-bounded application of a QoS Profile to a device (all traffic) or to identified application flows between a device and application server(s), depending on the API. | It explains temporary QoS treatment. |
...
| Connectivity quality booking | Provides confidence that the QoS Profile is usable by the requesting API consumer for a given time window and a defined service area. This confidence is strongest for a future-dated booking, where the CSP has lead time to confirm availability ahead of use; a booking requested for immediate start has little or no such lead time and behaves closer to an on-demand request. | It explains `qos-booking`, `qos-booking-and-assignment` and `dedicated-network`. Through the booking, the API consumer gets confidence that the QoS Profile is usable. The CSP decides whether it can be confirmed. The minimum time between a reservation request and its confirmation/activation is defined by the CSP. Device assignment may be included in the booking or managed separately, depending on the API. |
| Reservation lifecycle state | After an API consumer requests a reservation (via `qos-booking`, `qos-booking-and-assignment` or `dedicated-network`), the CSP accepts and commits the requested resources. This committed state is called _reserved_. The service remains unavailable until the scheduled activation time. Once activation time arrives, the state becomes _activated_. | It clarifies that API request success ≠ service activation. The CSP commitment (_reserved_ state) is distinct from activation (_activated_ state). This helps API consumers understand why a confirmed booking may not immediately provide service. |
| Service area | The geographic area where requested or booked connectivity quality is expected to apply. | It gives the API consumer an understandable geographic abstraction while keeping operator-internal topology hidden. **Note:** While there are currently some differences in how the reservation APIs implement the concept, alignment is ongoing across the CQM APIs to make the [Network Service Areas](https://github.com/camaraproject/NetworkServiceAreas) API a common solution. |
...
**Note on terminology:** This document describes the reservation lifecycle states in plain language. The state names in the API specifications differ, for example `RESERVED` in `dedicated-network` and `SCHEDULED` in `qos-booking`.
...
| A single-camera contribution is scheduled in advance at a known venue | Planning of the needed connectivity quality for the one device during a future time window and service area | `qos-booking`, `qos-booking-and-assignment` or `dedicated-network` to book the QoS Profile for a specific time window and service area in advance | Confidence that the connectivity quality for a known time and place is available. |
| A small outside broadcast uses several devices that may be replaced during the event | Reserve connectivity first and assign or re-assign devices later | `qos-booking-and-assignment` to book connectivity for a time window and service area and manage device assignment or re-assignment within the booking; `dedicated-network` with `dedicated-network-accesses` to book connectivity for a time window and service area and grant or revoke device access as devices are replaced | Decouple the reservation from the final device list. The relevant tool depends on whether the scenario requires device assignment only or the management of multiple devices and/or QoS Profiles. |
| A complex production uses live video, intercom, preview feeds and operational traffic in the same venue | Use a reserved connectivity environment where several QoS Profiles and device-access rules may be available | `dedicated-network` with `dedicated-network-profiles` and `dedicated-network-accesses` to reserve a multi-profile connectivity environment and manage device access — devices with granted access are served by the access's default QoS Profile; `dedicated-network-accesses` to scope QoS Profiles per device category (e.g. intercom devices vs. video cameras); `dedicated-network-areas` to select an eligible service area for the dedicated network; `quality-on-demand` where traffic needs a different QoS Profile from the network's set | Manage a reserved connectivity environment with multiple QoS Profiles and related conditions. Devices with granted access receive the default QoS Profile automatically. `quality-on-demand` may be used where supported by the CSP offering to apply a different QoS Profile dynamically. |
...
*Figure 2 — CQM APIs grouped by purpose. Actual API availability depends on CSP offering and market context.*
...
- **Connectivity quality booking** — to get confidence that the QoS Profile is usable for a given time window, which can start immediately or at a future point, and a defined service area:
  - `qos-booking`
...
Each API is explained through the need it addresses, what it controls or exposes, the API consumer takeaway, and what it should not be confused with. For a more detailed description at API endpoint and schema level, please refer to the individual API descriptions and/or definitions.
...
| Not to be confused with | A universal guarantee that identical measured performance will be available at every time and location, or a reservation for a future time window and service area. |
...
| What it controls | Device accesses associated with a `dedicated-network`. An access also selects a subset of the network profile's QoS Profiles and may set a different default QoS Profile for its devices, allowing different QoS Profile combinations for different device categories. |
| API consumer takeaway | This is the device-access companion to `dedicated-network`: it manages which devices may use the reserved connectivity environment and how the network profile's QoS capabilities are scoped per access. |
...
- whether one QoS Profile per booking is sufficient, or the scenario requires multiple QoS Profiles within one reserved connectivity environment
...
### 8.3 Sources and meta-release reference
...
Each API's repository and maturity status are documented in the table in [Section 1](#the-apis-within-the-connectivity-quality-management-subproject).
